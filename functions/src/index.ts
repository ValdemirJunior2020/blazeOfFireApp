...
export const notifyAdminOnPrayerRequest = onDocumentCreated(
  "prayer_requests/{requestId}",
  async (event: FirestoreCreateEvent) => {
    ...
    const tokens = devicesSnapshot.docs
      .map((docSnap) => docSnap.data() as AdminDeviceDoc)
